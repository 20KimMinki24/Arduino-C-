...
void loop() {
  ...
  // 센서 값을 바탕으로 모터 제어
  if (leftSensor == LOW || leftCenterSensor == LOW && (leftSensor == LOW || leftCenterSensor == LOW)){
   ...
   analogWrite(ENA, V);
   analogWrite(ENB, V);}
  if (rightSensor == LOW || rightCenterSensor == LOW && (rightSensor == LOW || rightCenterSensor == LOW)) {
   digitalWrite(IN1, HIGH);
   digitalWrite(IN2, LOW);
   digitalWrite(IN3, LOW);
   digitalWrite(IN4, HIGH);
   analogWrite(ENA, V);
   analogWrite(ENB, V);} else { moveForward(); }
}//void loop
